...
class FNN(Model):
    def __init__(self, field_sizes=None, embed_size=10, layer_sizes=None, layer_acts=None, drop_out=None,
        ...
        for i in range(num_inputs):
            layer_input = field_sizes[i]
            layer_output = embed_size
            init_vars.append(('embed_%d' % i, [layer_input, layer_output], 'xavier', dtype))
        ...
        with self.graph.as_default():
            if random_seed is not None:
                tf.set_random_seed(random_seed)
            self.X = [tf.sparse_placeholder(dtype, [None, field_sizes[i]]) for i in range(num_inputs)]
            self.y = tf.placeholder(dtype)
            ...
            l = tf.concat([xw[i] for i in range(num_inputs)], 1)
...
class CCPM(Model):
    def __init__(self, layer_sizes=None, layer_acts=None, drop_out=None, init_path=None, opt_algo='gd',
        ...
        init_vars.append(('f1', [embedding_order, layer_sizes[2], 1, 2], 'xavier', dtype))
        init_vars.append(('f2', [embedding_order, layer_sizes[3], 2, 2], 'xavier', dtype))
        init_vars.append(('w1', [2 * 3 * embedding_order, 1], 'xavier', dtype))
...
class PNN2(Model):
    def __init__(self, layer_sizes=None, layer_acts=None, drop_out=None, layer_l2=None, kernel_l2=None, init_path=None,
        ...
        for i in range(num_inputs):
            ...
            init_vars.append(('b0_%d' % i, [layer_output], 'zero', dtype))
        init_vars.append(('w1', [num_inputs * factor_order, layer_sizes[2]], 'xavier', dtype))
        init_vars.append(('k1', [factor_order * factor_order, layer_sizes[2]], 'xavier', dtype))
        init_vars.append(('b1', [layer_sizes[2]], 'zero', dtype))
        for i in range(2, len(layer_sizes) - 1):
            layer_input = layer_sizes[i]
            layer_output = layer_sizes[i + 1]
